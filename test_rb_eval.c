...
int all_found(polynomial* p, polynomial* dp, polynomial* init, complexe** roots, int nb_root_found, int nb_root, int ITERATION_LOOP, double EPS, double w, int* nm_iteration_tot){
    //Creating the set Sd
    int Sd_size;
    double upper_bound = radius_complex_hull(p);
    complexe** Sd = set(p->degree, &Sd_size, upper_bound);
    ...
    while (nb_root_found < nb_root && completed < Sd_size && nm_iteration < max_nm_iteration){
        ...
        while(i < set_size && nb_root_found < nb_root && completed < Sd_size){
            ...
            do {
                ...
                nm_iteration++;
                k++;
            } while (k < ITERATION_LOOP && evaluate_mod(init, z0) > EPS);
            
            if(nb_root_found < nb_root && evaluate_mod(init, z0) <= EPS){
                completed++;
...
int root_finder(char* TEST_FILE, FILE* OUTPUT_FILE, int ITERATION_LOOP, double EPS, double relaxation){
    ...
    for(int test = 0; test < t->nb_test; test ++){
        ...
        while (nb_root_found < nb_root && simple_roots->degree >= 3){
            int new_nb_root_found = all_found(simple_roots, dsr, p, roots, nb_root_found, nb_root, ITERATION_LOOP, EPS, w, &nm_iteration);
            ...
            polynomial* root_found_prod = develop(&roots[nb_root_found], found);
        ...
        if(nb_root_found == nb_root){
            for(int i = 0; i < nb_root; i++){
                double r = evaluate_mod(p, roots[i]);
                if(r > max_error){
                    max_error = r;
                }
            }
        }else{
            printf("Problème : %d racines trouvées sur %d.\n", nb_root_found, nb_root);
        }
        
        free_complexe_array(roots, nb_root);
        free_polynomial(p);

        fprintf(OUTPUT_FILE,"%d;%.12lf;%d;%d;%lf\n",ITERATION_LOOP,max_error,nb_root_found,nm_iteration,duration);
    }
